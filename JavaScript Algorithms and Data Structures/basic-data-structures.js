...
// // 'nestedObject' has three properties: 'id' (value is a number), 'date' (value is a string), and 'data' (value is an object with its nested structure). While structures can quickly become complex, we can still use the same notations to access the informaiton we need. To assign the value '10' to the 'busy' property of the nested 'onlineStatus' object, we use dot notation to reference the property:
...
// ## Access Property Names iwth Bracket Notation

// In the first object challenge we mentioned the use of bracket notation as a way to access property values using the evaluation of a variable. For instance, imagine that our 'foods' object is being used in a program for a supermarkett cash register. We have some function that sets the 'selectedFood' and we want to check our 'food's object for the presence of that food. This might look like: 

// let selectedFood = getCurrentFood(scannedItem);
// let inventory = foods[selectedFood];

// This code will evaluate the value stored in the 'selectedFood' variable and return the value of that key in the 'foods' object, or 'undefined' if it is not present. Bracket notation is very usefull because sometimes object properties are not known before runtime or we need to access them in a more dynamic way.
...
// Now you know what objects are an their basic features and advantages. In short, they are key-value stores which provide a flexible, intuitive way to structure data, and, they provide very fast lookup time. Throughout the rest of these challenges, we will describe several common operations you can perform on objects so you can become comfortable applying these useful data structures in your programs.

// In earlier challenges, we have both added to and modified an object's key-value pairs. Here we will see how we can remove a key-value pair from an object.

// Let's revisit our 'foods' object example on last time. If we wanted to remove the 'apples' key, we can remove it by using the 'delete' keyword like this:
...
// Now we can add, modify, and remove keys from objects. But what if we just wanted to know if an object has a specific property? JavaScript provides us with two different ways to do this. One uses the 'hasOwnProperty()' method ane the other uses the 'in' keyword. If we have an object 'users' with a property of 'Alan', we could check for its presence in either of the following ways:
...
// We've defined a function 'countOnline' which accepts one argument (a users object). Use a for...in statement within this function to loop through the users object passed into the function and return the number of users whose 'online' property is set to 'true'. An example fo a users object which could be passed to 'countOnline' is shown below. Each user will have an 'online' property with either a 'true' or 'false' value.
...
//         if(usersObj[user]['online']) onlineCount++;
//     } return onlineCount;
//     // Only change code above this line
// }
...
// We can also generate an array which contains all the keys stored in an object with the 'Object.keys()' method. This method takes an object as the argument and returns an array of strings representing each property in the object. Again, there will be specific order to the entries in the array.
...
// Now you've seen all the basic operations for JavaScript objects. You can add, modify, and remove key-value pairs, check if keys exist, and iterate over all the keys in an object. As you continue learning JavaScript you will see even more versatile applications of objects. Additionally, the Data Structures lessons located in the Coding Interview Prep of the curriculum also cover the ES6 Map and Set objects, both of which are similar to ordinary objects but provide some additional features. Now that you've learned the basics of arrays and objects, you're fully prepared to begin tacking more complex problems using JavaScript!
